...
#[tokio::test]
async fn test_compact_format_lifecycle() {
    // Initialize logging for tests
    let _ = tracing_subscriber::fmt()
        .with_max_level(tracing::Level::DEBUG)
        .try_init();
    tracing::info!("Starting compact format lifecycle test");
    ...
    // Configure feed with event fields for compact format
    let config = FeedAcceptConfig {
        accept_aggregation_period: Some(1000.0),
        accept_data_format: Some(FeedDataFormat::Compact),
        accept_event_fields: Some(HashMap::from([
    ...
    // Add a small delay to ensure configuration is processed
    tokio::time::sleep(Duration::from_millis(500)).await;
        ...
        if let Ok(Ok(event)) = result {
            match event {
                FeedEvent::Quote(quote) => {
                    tracing::info!("Received quote event: {:?}", quote);
                    assert_eq!(quote.event_symbol, "AAPL");
                    // Verify we have the expected fields
                    assert!(quote.bid_price.is_some() || quote.ask_price.is_some());
                    assert!(quote.bid_size.is_some() || quote.ask_size.is_some());
                    received_data = true;
                },
                _ => {
