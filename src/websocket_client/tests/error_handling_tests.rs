...
use crate::{DxLinkError, DxLinkErrorType, DxLinkWebSocketClientConfig};
use std::sync::Arc;
...
async fn test_enhanced_error_message_handling() {
    ...
    client
        .add_error_listener(Box::new(move |error| {
    ...
    assert!(
        errors[0].message.starts_with("Connection error: "),
        "Expected message to start with 'Connection error:', got: {}",
        errors[0].message
    );
    assert!(
        errors[0]
            .message
            .contains("Connection timed out after 30 seconds"),
